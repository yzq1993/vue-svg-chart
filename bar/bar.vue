<template>
  <div class="main">
      ...
      <g>
        <g v-for="item in pathList">
          <rect v-for="child in item.rect" :x=child.x :y=child.y :width=child.width :height=child.height :fill=item.color :fill-opacity="item.hover?1:0.75" 
          :stroke=item.color></rect>
        </g>
      </g>
      <g>
...
export default {
  ...
  computed:{
    ...
    pathList:function(){
      let table_height=this.height-this.tmpPadding[0]-this.tmpPadding[2],height,
        width=(this.grid.width-this.space*(this.list.length+1))/this.list.length
      return this.list.map((v,i)=>{
        return {
          ...v,
          rect:this.label.map((m,j)=>{
            height=table_height*(v.data[j]-this.min)/(this.max-this.min)
            return {
              x:this.origin.x+j*this.grid.width+i*width+this.space*(i+1),
              y:this.origin.y-height,
              width:width,
              height:height
            }
          })
